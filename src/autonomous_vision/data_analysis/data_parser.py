...
from autonomous_vision.data_analysis.bdd_to_coco import main as coco_main
from autonomous_vision.data_analysis.parser_core import main as parser_main
...
def main():
    ...
    print("Starting BDD100K data processing pipeline...")
    ...
    if not parser_success or not coco_success:
        ...
        if not parser_success:
            print(
                ...
                "autonomous_vision.data_analysis.parser_core"
            )
        if not coco_success:
            print(
                "Run COCO: uv run python -m "
                "autonomous_vision.data_analysis.bdd_to_coco"
            )

        print(
            "Run both: uv run python -m "
            "autonomous_vision.data_analysis.data_parser"
        )
